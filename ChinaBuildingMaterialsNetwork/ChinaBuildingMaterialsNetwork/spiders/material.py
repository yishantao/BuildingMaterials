...
class MaterialSpider(scrapy.Spider):
    ...
    def parse_company(self, response):
        ...
        le = LinkExtractor(restrict_xpaths='//li[@id="nav-news"]/a')
        links = le.extract_links(response)
        if links:
            for link in links:
                yield Request(link.url, callback=self.parse_news_total)
    ...
    def parse_news_total(self, response):
        total_page = response.xpath('//span[@class="paginationInfo"]/text()').re_first('/\d*')
        if total_page:
    ...
    def parse_news_page(self, response):
        le = LinkExtractor(restrict_xpaths='//div[@class="news-item"]/ul/li/h3/a')
        links = le.extract_links(response)
        if links:
            for link in links:
                yield Request(link.url, callback=self.parse_news)

    def parse_news(self, response):
        company = {'_id': response.xpath('//div[@class="head-product"]/h2/text()').extract_first(),
                   'news': {}}
        title = response.xpath('//div[@class="m-news-info"]/h1/text()').extract_first()
        if title:
            company['news']['title'] = title
        time = response.xpath('//div[@class="m-news-info"]/div[@class="detail"]/p/span[1]').extract_first()
        if time:
            company['news']['time'] = time
        source = response.xpath('//div[@class="m-news-info"]/div[@class="detail"]/p/span[2]').extract_first()
        if source:
            source = source.split('：')[-1]
            company['news']['source'] = source
        text = response.xpath('//div[@class="newsinfo_cont"]/*//text()').extract()
        if text:
            company['news']['text'] = []
            for i in text:
                if i.strip():
                    company['news']['text'].append(i)

        company['news']['photos'] = []
        image_list = response.xpath('//div[@class="newsinfo_cont"]//img/@src').extract()
        if image_list:
            item = ImageItem()
            item['image_urls'] = []
            for photo in image_list:
                ...
                image_detailed_url = {'original_url': original_url,
                                      'local_url': local_url}
                company['news']['photos'].append(image_detailed_url)
                item['image_urls'].append(original_url)
